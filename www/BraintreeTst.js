 ...
 */
BraintreeTst.initialize = function initialize(token, successCallback, failureCallback) {
 ...
 * @param [function] failureCallback - The failure callback for this asynchronous function; receives an error string.
 */
BraintreeTst.setupApplePay = function setupApplePay(options, successCallback, failureCallback) {
    if (!options) {
        options = {};
    }
 ...
 * @param [function] failureCallback - The failure callback for this asynchronous function; receives an error string.
 */
BraintreeTst.presentDropInPaymentUI = function showDropInUI(options, successCallback, failureCallback) {
...
BraintreeTst.paypalProcess = function paypalProcess(amount, currency, env, successCallback, failureCallback) {
    exec(successCallback, failureCallback, PLUGIN_ID, 'paypalProcess', [amount, currency, env]);
};

BraintreeTst.paypalProcessVaulted = function paypalProcessVaulted(env, successCallback, failureCallback) {
    exec(successCallback, failureCallback, PLUGIN_ID, 'paypalProcessVaulted', [env]);
};


module.exports = BraintreeTst;
